...
import { wrapErr, wrapOk, sanitize } from "../lib/parser.js";
...
const inputSchema = {
    mode: z.enum(MODES).describe("Skill or utility mode: understand | investigate | impact | plan | debug | architecture | docs | locate | status"),
    targets: z.array(z.string().min(1)).optional().describe("File paths, symbol names, or identifiers to scope the query"),
    query: z.string().min(1).optional().describe("Natural language question or search terms"),
    depth: z.enum(["shallow", "medium", "deep"]).optional().describe("Reasoning depth (understand only); defaults to medium"),
    freshness_policy: z
...
export function register(server) {
    registerIxTool(server, {
        name: TOOL_NAME,
        description: "Unified ix query tool. Routes to the appropriate Ix Core Runtime pipeline based on mode: " +
...
async function runQuery(input) {
    ...
    const response = await queryRuntime(body);
    if (!response.ok) {
    ...
    const evidence = Array.isArray(raw.evidence)
        ...
        : undefined;
    return wrapOk(TOOL_NAME, input, data, summary, preview_markdown, canonical_revision, evidence, response.duration_ms);
}
...
function buildFallbackPreview(mode, raw) {
    if (raw.summary)
        return `**${mode}**: ${raw.summary}`;
    if (raw.stale)
        return `**${mode}**: Index may be stale — results could be incomplete.`;
    return `**${mode}**: Query completed.`;
}
async function fallbackQueryCli(input) {
    ...
    switch (input.mode) {
        case "status": {
            ...
            const revision = typeof raw["currentRev"] === "number" ? raw["currentRev"] : null;
            const staleFiles = typeof raw["staleFiles"] === "number" ? raw["staleFiles"] : 0;
            return wrapOk(TOOL_NAME, input, { backend: raw["backend"] ?? null, current_rev: revision, stale_files: staleFiles, fallback: "cli" }, summarizeMode(input), `## ix_query: status\n\n**Backend:** ${String(raw["backend"] ?? "unknown")}\n**Revision:** ${revision ?? "unknown"}\n**Stale files:** ${staleFiles}`, revision, undefined, result.durationMs);
        }
        case "understand":
        case "architecture": {
            ...
            return wrapOk(TOOL_NAME, input, { mode: input.mode, regions: raw["regions"] ?? [], fallback: "cli" }, summarizeMode(input), `## ix_query: ${input.mode}\n\n**Top subsystems:** ${names.join(", ") || "none"}`, typeof raw["map_rev"] === "number" ? raw["map_rev"] : null, undefined, result.durationMs);
        }
        case "investigate": {
            if (!target)
                return null;
            ...
            return wrapOk(TOOL_NAME, input, { mode: input.mode, data: raw, fallback: "cli" }, typeof explanation === "string" ? explanation : summarizeMode(input), `## ix_query: investigate\n\n${typeof explanation === "string" ? explanation : `Investigated ${target}`}`, null, undefined, result.durationMs);
        }
        case "impact": {
            if (!target)
                return null;
            ...
            return wrapOk(TOOL_NAME, input, { mode: input.mode, data: raw, fallback: "cli" }, summarizeMode(input), `## ix_query: impact\n\n**Risk:** ${riskLevel.toUpperCase()}${typeof riskSummary === "string" ? `\n**Summary:** ${riskSummary}` : ""}`, null, undefined, result.durationMs);
        }
        case "locate": {
            ...
            if (!result.ok)
                return null;
            const raw = parseIxJson(result.stdout);
            return wrapOk(TOOL_NAME, input, { mode: input.mode, data: raw, fallback: "cli" }, summarizeMode(input), `## ix_query: locate\n\nResolved or searched for \`${term}\`.`, null, undefined, result.durationMs);
        }
        default:
            return null;
    }
}
//# sourceMappingURL=ix_query.js.map