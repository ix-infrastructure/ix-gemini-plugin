...
export function timeoutFor(endpoint: string): number {
  if (endpoint.includes("ingest/map")) return TIMEOUT_INGEST_MS;
  if (endpoint.includes("status")) return TIMEOUT_STATUS_MS;
  if (endpoint.includes("ix_decide")) return TIMEOUT_DECIDE_MS;
