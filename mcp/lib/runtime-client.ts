...
export type RuntimeResponse<T> =
  | { ok: true; data: T; canonical_revision: number | null; duration_ms: number }
  | { ok: false; code: string; message: string; duration_ms: number };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (typeof errBody["message"] === "string") errorMessage = errBody["message"];
      } catch {
    ...
    const canonical_revision =
      typeof (data as Record<string, unknown>)["canonical_revision"] === "number"
        ? ((data as Record<string, unknown>)["canonical_revision"] as number)
        : null;
...
export async function queryRuntime<T>(body: RuntimeRequest): Promise<RuntimeResponse<T>> {
  return postJson<T>("/v2/ix_query", body);
}

export async function decideRuntime<T>(body: RuntimeRequest): Promise<RuntimeResponse<T>> {
  return postJson<T>("/v2/ix_decide", body);
}

export async function ingestRuntime<T>(body: RuntimeRequest): Promise<RuntimeResponse<T>> {
  return postJson<T>("/v2/ingest/map", body);
}

export async function statusRuntime<T>(): Promise<RuntimeResponse<T>> {
  return getJson<T>("/v2/status");
}

export async function graphQueryRuntime<T>(body: RuntimeRequest): Promise<RuntimeResponse<T>> {
  return postJson<T>("/v2/graph/query", body);
}

export async function insightsDeriveRuntime<T>(body: RuntimeRequest): Promise<RuntimeResponse<T>> {
  return postJson<T>("/v2/insights/derive", body);
}

export async function briefingRuntime<T>(): Promise<RuntimeResponse<T>> {
  return getJson<T>("/v2/briefing");
}
