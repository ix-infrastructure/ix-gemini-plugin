...
test("wrapOk redacts secrets across all model-facing fields", () => {
  const result = wrapOk(
    "ix_query",
    { query: SECRET, nested: { token: SECRET } },
    { summary: SECRET, evidence: [{ detail: SECRET }] },
    `summary ${SECRET}`,
    `preview ${SECRET}`,
    42,
    [{ kind: "graph", id: "node-1", name: `name ${SECRET}`, confidence: "observed" }],
    12,
  );
...
test("wrapErr redacts secrets in input and message", () => {
  const result = wrapErr(
    "ix_query",
    { query: SECRET },
    { code: "IX_UPSTREAM_UNAVAILABLE", message: `runtime rejected ${SECRET}` },
  );
