...
import { wrapErr, wrapOk, type Evidence, type ToolResult } from "../lib/parser.js";
...
const inputSchema = {
  paths: z.array(z.string().min(1)).min(1).describe(
    "File paths that will be written or edited",
  ),
  operation: z.enum(["write", "edit", "delete"]).describe(
    "The type of file operation about to be performed",
  ),
  context: z.string().optional().describe(
    "Brief description of why this change is being made; helps the policy engine",
  ),
  freshness_policy: z
...
type DecideInput = ToolInput<typeof inputSchema>;
...
export function register(server: McpServer): void {
  registerIxTool(server, {
    name: TOOL_NAME,
    description:
      "Pre-edit policy gate. Call this before writing, editing, or deleting any file. " +
...
async function runDecide(input: DecideInput): Promise<ToolResult> {
  ...
  const response = await decideRuntime<DecideResponse>(body);
  ...
  if (!response.ok) {
    ...
    if (cliResult.ok) {
      ...
      const verdict: Verdict = stale ? "warn" : "allow";
      const reason = stale
      ...
      return wrapOk(
        ...
        `ix_decide: ${verdict} — ${reason.slice(0, 120)}`,
        preview_markdown,
        revision,
        undefined,
    ...
    const preview_markdown =
      `**ix_decide**: Runtime unavailable — defaulting to **allow**.\n\n` +
      `Proceed with caution: Ix graph context is not available for this decision.\n\n` +
      `Error: ${response.message}`;
    return wrapOk(
      TOOL_NAME,
      input as unknown as Record<string, unknown>,
      { verdict: "allow", reason: `Runtime unavailable: ${response.message}`, runtime_available: false },
      "Runtime unavailable — defaulting to allow",
      preview_markdown,
      null,
      undefined,
...
function buildDecidePreview(verdict: Verdict, reason: string, paths: string[]): string {
