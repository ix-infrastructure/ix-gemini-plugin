...
async function runSubsystems(input: SubsystemsInput): Promise<ToolResult> {
  const response = await graphQueryRuntime<SubsystemsResponse>({ operation: "subsystems" });

  if (!response.ok) {
    const cliResult = await runIx(["subsystems"]);
    if (cliResult.ok) {
      const raw = parseIxJson(cliResult.stdout) as SubsystemsResponse;
      ...
      return wrapOk(
        ...
        raw.preview_markdown ?? buildSubsystemsPreview(subsystems),
        raw.canonical_revision ?? raw.map_rev ?? null,
        undefined,
    ...
    return wrapOk(
      TOOL_NAME,
      input as unknown as Record<string, unknown>,
      { subsystems: [], runtime_available: false },
      "Runtime unavailable — no subsystems",
      `## ix_subsystems: Unavailable\n\n**Error:** ${response.message}`,
      null,
      undefined,
...
function inferPurpose(region: Region): string {
  ...
  if (signals.length === 0) return `${capitalize(kind)} inferred from graph clustering`;
...
function buildSubsystemsPreview(subsystems: Array<{ name: string; files: number; kind: string | null }>): string {
  if (subsystems.length === 0) return `## ix_subsystems\n\nNo subsystems found.`;
  ...
  if (subsystems.length > 15) lines.push(`\n_...and ${subsystems.length - 15} more_`);
  return lines.join("\n");
}
